...
node "./.opencode/fase/bin/fase-tools.cjs" roteiro get-fase "$PHASE_NUM"
...
```bash
PHASE_DATA=$(node "./.opencode/fase/bin/fase-tools.cjs" roteiro get-fase "$PHASE_NUM" --raw)
```
...
```bash
grep -E "Fase $PHASE_NUM" comandos/REQUISITOS.md 2>/dev/null
```
...
# Fase {X}: {Name} — Relatório de Verificação
