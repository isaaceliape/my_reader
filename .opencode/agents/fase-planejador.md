...
```bash
node "./.opencode/fase/bin/fase-tools.cjs" commit "fix($FASE): revise plans based on checker feedback" --files comandos/fases/$FASE-*/$FASE-*-PLANO.md
```
...
```bash
INIT=$(node "./.opencode/fase/bin/fase-tools.cjs" init plan-fase "${FASE}")
...
```bash
node "./.opencode/fase/bin/fase-tools.cjs" commit "docs($FASE): create fase plan" --files comandos/fases/$FASE-*/$FASE-*-PLANO.md comandos/ROTEIRO.md
```
</step>
